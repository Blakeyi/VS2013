...
CButtonST::~CButtonST()
{
	...
	if (m_dcBk.m_hDC && m_pbmpOldBk->m_hObject)
	{
...
#else
	if (IsMenu(m_hMenu) && m_hMenu) ::DestroyMenu(m_hMenu);
#endif
} // End of ~CButtonST
	...
	switch (m_byAlign)
	{
		case ST_ALIGN_HORIZ:
			...
			else
			{
				...
				//rpImage->left += 6;  
				//rpTitle->left += dwWidth -63; 
				rpImage->left += 1;  
				//rpTitle->left += dwWidth +1; 
				rpTitle->top +=0;//按钮上有文字时调整文字位置
			}
