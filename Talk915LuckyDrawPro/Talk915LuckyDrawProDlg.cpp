...
BOOL CTalk915LuckyDrawProDlg::OnInitDialog()
{
	...
	int nNameW = rcClient.Width() * 0.26;
	int nNameH = rcClient.Height() * 0.097;
	int nBtnClose = 20;

	GetDlgItem(IDC_BUTTON_START)->SetWindowPos(NULL, (rcClient.Width() - nBtnW) / 2, rcClient.Height() * 0.85, nBtnW, nBtnH, SWP_SHOWWINDOW);
	GetDlgItem(IDC_STATIC_NAME)->SetWindowPos(NULL, (rcClient.Width() - nNameW) / 2, rcClient.Height() * 0.511, nNameW, nNameH, SWP_SHOWWINDOW);
	...
	m_font.CreatePointFont(800, _T("微软雅黑"));
...
void CTalk915LuckyDrawProDlg::OnBnClickedButtonStart()
{
	...
	else
	{
		...
		StartThread();
		////list<CString> iter;
		////random_shuffle(m_listNames.begin(), m_listNames.end());
		//list<CString>::iterator iter = m_listNames.begin();
		//while (m_bRoll)
		//{
		//	for (; iter!=m_listNames.end(); iter++)
		//	{
		//		GetDlgItem(IDC_STATIC_NAME)->SetWindowText(*iter);
		//		CRect crc;
